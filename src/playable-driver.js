...
import { NET, TABLE, clamp } from '../serve/src/shared/game-core.js';
...
export class PlayableDriver {
  ...
  mapEventSideToRacket(side) { return side === 'p1' ? 'player' : 'ai'; }

  processGameEvent(event, options = {}) {
    if (!event) return;
    if (event.type === 'shot' || event.type === 'hit') {
      triggerPaddleHitAnimation(this, event.side, { smash: event.smash, contact: event.contact });
    }
  ...
  syncPlayStore(state, { sideToStore = (side) => side, extra = {}, setter = 'setState' } = {}) {
    ...
    const changed = patch.scoreP !== store.scoreP
      || patch.scoreAI !== store.scoreAI
      || patch.phase !== store.phase
      || patch.server !== store.server
      || patch.winner !== store.winner;
    if (setter === 'syncOnlineState') store.syncOnlineState(patch);
    else if (changed) useGameStore.setState(patch);
  }
