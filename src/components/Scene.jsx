...
const lineJoinOverlap = edgeLineWidth;
...
export function TableModel() {
  ...
  return (
    <group>
      ...
      <TableLine w={edgeLineWidth} d={lineZ * 2 + lineJoinOverlap} x={-lineX} z={0} matRef={(node) => { lines.current[0] = node; }} />
      <TableLine w={edgeLineWidth} d={lineZ * 2 + lineJoinOverlap} x={lineX} z={0} matRef={(node) => { lines.current[1] = node; }} />
      <TableLine w={lineX * 2 + lineJoinOverlap} d={edgeLineWidth} x={0} z={-lineZ} matRef={(node) => { lines.current[2] = node; }} />
      <TableLine w={lineX * 2 + lineJoinOverlap} d={edgeLineWidth} x={0} z={lineZ} matRef={(node) => { lines.current[3] = node; }} />
...
function Text3DMaterial({ color, sideColor, unlit = false }) {
  if (unlit) {
    return (
      <>
        <meshBasicMaterial attach="material-0" color={color} toneMapped={false} />
        <meshBasicMaterial attach="material-1" color={sideColor} toneMapped={false} />
      </>
    );
  }
  return (
    <>
      <meshStandardMaterial attach="material-0" color={color} roughness={0.72} metalness={0} />
      <meshStandardMaterial attach="material-1" color={sideColor} roughness={0.82} metalness={0} />
    </>
  );
}
const textBounds = new Box3();
const textCenter = new Vector3();
function WallText3D({ children, fontSize, height, bevelSize, color, sideColor, unlit = false }) {
  ...
  useLayoutEffect(() => {
    ...
    textBounds.copy(text.geometry.boundingBox);
    textBounds.getCenter(textCenter);
    text.position.set(-textCenter.x, -textCenter.y, 0);
  ...
  return (
    ...
    >
      {children}
      <Text3DMaterial color={color} sideColor={sideColor} unlit={unlit} />
    </Text3D>
  );
}
function applyLabelText(node, config, active, letterSpacing = 0) {
  ...
  node.outlineWidth = config.labelOutlineWidth;
  node.outlineBlur = config.labelOutlineBlur;
...
export function WallScoreboard() {
  ...
  const playerScoreText = String(scoreP).padStart(2, '0');
  const cpuScoreText = String(scoreAI).padStart(2, '0');
  ...
  useEffect(() => {
    if (roomCodeText.current) roomCodeText.current.visible = mode === 'online' && networkStatus === 'waiting' && !!roomCode;
    lastServer.current = null;
  }, [mode, networkStatus, roomCode]);
  ...
  useFrame((state, delta) => {
    ...
    const breathe = mode === 'replay' ? 1 : 1 + Math.sin(state.clock.elapsedTime * 0.7) * config.breatheAmp + arenaFx.heat * config.breatheHeat;
  ...
  return (
    <group frustumCulled={false} visible={started}>
      <group ref={playerScore} renderOrder={4}>
        <WallText3D key={playerScoreText} fontSize={TUNING.scoreboard.scoreFontSize} height={TUNING.scoreboard.text3dDepth} bevelSize={TUNING.scoreboard.text3dBevel} color={TUNING.scoreboard.scoreFill} sideColor={TUNING.scoreboard.text3dSide} unlit={mode === 'replay'}>
          {playerScoreText}
        </WallText3D>
      </group>
      <group ref={cpuScore} renderOrder={4}>
        <WallText3D key={cpuScoreText} fontSize={TUNING.scoreboard.scoreFontSize} height={TUNING.scoreboard.text3dDepth} bevelSize={TUNING.scoreboard.text3dBevel} color={TUNING.scoreboard.scoreFill} sideColor={TUNING.scoreboard.text3dSide} unlit={mode === 'replay'}>
          {cpuScoreText}
        </WallText3D>
      </group>
      <group ref={roomCodeText} renderOrder={6} visible={mode === 'online' && networkStatus === 'waiting' && !!roomCode}>
        <WallText3D fontSize={TUNING.scoreboard.roomCodeFontSize} height={TUNING.scoreboard.text3dRoomDepth} bevelSize={TUNING.scoreboard.text3dRoomBevel} color={TUNING.scoreboard.roomCodeFill} sideColor={TUNING.scoreboard.text3dSide}>
          {`ROOM ${roomCode}`}
        </WallText3D>
      </group>
