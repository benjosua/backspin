...
import { COLORS, DEFAULT_DIFFICULTY, DEFAULT_RACKET_COLOR, PLAYER_SPEED } from './constants.js';
...
const resetOnlineState = () => ({
  ...
  opponentRacketColor: DEFAULT_RACKET_COLOR,
});
...
export const useGameStore = create((set, get) => ({
  ...
  opponentName: 'OPPONENT',
  playerRacketColor: DEFAULT_RACKET_COLOR,
  opponentRacketColor: DEFAULT_RACKET_COLOR,
  ...
  setRankedProfile: (rankedProfile) => set({ rankedProfile, playerRacketColor: rankedProfile?.selectedRacketColor || DEFAULT_RACKET_COLOR }),
  setLeaderboard: (leaderboard) => set({ leaderboard }),
  setRankedQueueCount: (rankedQueueCount) => set({ rankedQueueCount }),
  setLivePlayerCount: (livePlayerCount) => set({ livePlayerCount }),
  setOnlineRematchRequested: (onlineRematchRequested) => set({ onlineRematchRequested }),
  setReplayLoading: () => set({ replayStatus: 'loading', replayError: '', replayPlaying: false, scoreP: 0, scoreAI: 0, phase: 'serve', winner: null }),
  setReplayError: (replayError) => set({ replayStatus: 'error', replayError: replayError || 'Replay failed', replayPlaying: false }),
  startReplayMode: ({ match, stats, viewerSide, durationMs, scoreP = 0, scoreAI = 0 }) =>
    set({
      started: true,
      mode: 'replay',
      networkStatus: 'idle',
  ...
  syncOnlineState: (next) => set((state) => state.mode === 'replay' ? {} : ({ ...next, flashId: next.phase === 'point' && state.phase !== 'point' ? state.flashId + 1 : state.flashId })),
