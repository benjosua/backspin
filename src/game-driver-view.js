...
export function syncCoreSample(driver, core, visualLeadSeconds = 0) {
  const visualNowMs = core.nowMs + Math.max(0, visualLeadSeconds) * 1000;
  const sample = core.phase === 'serve' ? serveBallForCore(core) : sampleBallPlan(core.ballPlan, visualNowMs);
...
export function isIncoming(state = {}, viewerSide = 'p1') {
  const phase = state.phase ?? 'exchange';
  if (phase !== 'exchange') return false;
  const lastHitter = state.lastHitter;
  if (lastHitter) return lastHitter !== viewerSide;
  const velocity = state.velocity || state.vel || state;
  const vz = Number(velocity.z ?? velocity.vz ?? state.ballVz ?? 0);
  return viewerSide === 'p2' || viewerSide === 'ai' ? vz < 0 : vz > 0;
}
...
export function updatePaddlePose(racket, { dt, incoming = false, ballY, replay = false } = {}) {
  const sign = racket.who === 'player' ? 1 : -1;
  const restY = racket.restY ?? makeRacket(racket.who, racket.baseZ).restY;
  const restRotX = racket.restRotX ?? makeRacket(racket.who, racket.baseZ).restRotX;
  const maxY = racket.who === 'player' ? 2.6 : 1.8;
  const targetBallY = ballY ?? restY;
  racket.swing = Math.max(0, racket.swing - dt * 4.5);
  racket.flash = Math.max(0, racket.flash - dt * 4);
  if (replay) {
    racket.y = damp(racket.y, incoming ? clamp(targetBallY, 0.42, maxY) : restY, 8, dt);
    racket.z = racket.baseZ - sign * racket.swing * 0.45;
    racket.rotX = restRotX - sign * racket.swing * 0.6;
...
export function triggerPaddleHitAnimation(driver, side, { smash = false, contact = null } = {}) {
  const visualSide = typeof driver.mapEventSideToRacket === 'function'
    ? driver.mapEventSideToRacket(side)
    : side === 'p1' ? 'player' : 'ai';
  const racket = visualSide === 'player' ? driver.player : driver.ai;
  if (!racket) return;
  const amount = smash ? 1 : 0.72;
  racket.swing = Math.max(racket.swing || 0, amount);
  racket.flash = Math.max(racket.flash || 0, amount);
  if (contact && Number.isFinite(contact.y)) {
    racket.y = clamp(contact.y, 0.42, racket.who === 'player' ? 2.6 : 1.8);
  }
}
