...
export class RallyRoom extends Room<{ state: RallyState }> {
  ...
  private handleProfile(client: Client, message: any) {
    ...
    const paddle = String(message?.paddle || "ember").slice(0, 24);
    if (side === "p1") { this.state.p1Name = name; this.state.p1Paddle = paddle; }
    else { this.state.p2Name = name; this.state.p2Paddle = paddle; }
  }
  ...
  private point(winner: Side, reason: string) {
    if (this.state.phase !== "rally" && this.state.phase !== "serve") return;
  ...
  private update(dtRaw: number) {
    ...
    for (const [sessionId, input] of this.inputs) {
      ...
      const vx = clamp((input.targetX - current) * 10, -PADDLE_SPEED, PADDLE_SPEED);
